...
def register(
    name: str, cls: Type[gym.Env], max_episode_steps=None, default_kwargs: dict = None
):
    """Register a ManiSkill2 environment."""
    if name in REGISTERED_ENVS:
        logger.warn(f"Env {name} already registered")
    if not issubclass(cls, gym.Env):
        raise TypeError(f"Env {name} must inherit from gym.Env")
