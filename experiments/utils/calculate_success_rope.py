...
def is_rope_success(state, state_init):
    routed = False
    ...
    # Cube given as (center, half_size)
    center = np.array([0.62, 0.05, 0.0])
    ...
    out = count_xz_plane_intersections(
        x_np, springs_np, (bbox_min, bbox_max),
    )

    if out['y_min_count'] >= 100 and out['y_max_count'] >= 100:
        routed = True
...
if __name__ == '__main__':
    ...
    for episode_dir in episode_dirs:
        ...
        for state_file in state_files:
            ...
            if int(state_file.split('/')[-1].split('.')[0]) < 800:  # last 100 frames, total 900
                continue
            with open(state_file, 'rb') as f:
                state = pkl.load(f)
            routed = is_rope_success(state, state_init)
            routed_count += routed * 1.0
            if routed_count >= 30:
                routed_success = True
        routed_success_list.append(routed_success)

    print("insert_rope success list:", routed_success_list)

    success = np.zeros((len(episode_dirs) + 2), dtype=int)
    success[:-2] = np.array(routed_success_list, dtype=int)
    success[-2] = success[:-2].sum()
    success[-1] = success[:-2].mean() * 100

    np.savetxt(Path(data_dir) / 'success.txt', success, fmt='%d')
    print(f'insert_rope success rate: {success[-2]} / {len(episode_dirs)} = {success[-1]:.1f}%')
