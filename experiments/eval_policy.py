...
def main(cfg):
    ...
    for episode_id in range(cfg.policy.n_episodes):
        ...
        env = gym.make(cfg.env_name, max_episode_steps=frame_rate * duration + 30, cfg=cfg, 
            obs_mode=cfg.obs_mode, exp_root=cfg.exp_root, randomize=True)
        ...
        while not (done or truncated):
            ...
            if cfg.env.robot.use_pusher:
                ...
                eef_rot[:, 1] *= -1
                eef_rot[:, 2] *= -1
                eef_rot = eef_rot.unsqueeze(0).repeat(eef_xyz.shape[0], 1, 1)
                eef_quat = kornia.geometry.conversions.rotation_matrix_to_quaternion(eef_rot)
                eef_gripper = torch.zeros_like(cartesian_action[:, :1])  # always open
            else:
                eef_xyz = cartesian_action[:, :3]
